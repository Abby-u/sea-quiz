...
function animx.newAnimation(params)
	...
	if type(params)=='string' then
		...
		delay=0.0416666667 --params.delay or params.interval or .1
...
function animx.newAnimationXML(image,filename)
	...
	for line in love.filesystem.lines(filename) do
		if i>1 and line:match('%a') and not line:match('<!') and line~="</TextureAtlas>" then
			local frameNoo = string.match(line, 'name=".-(%d%d%d%d)"')
			frameNo=tonumber(frameNoo)+1
			...
			local _, width = string.match(line, "w=([\"'])(.-)%1")
			local _, height = string.match(line, "h=([\"'])(.-)%1")
...
function animx.newActorXML(image,filename)
	...
	for line in love.filesystem.lines(filename) do
		if i>1 and line:match('%a') and not line:match('<!') and line~="</TextureAtlas>" then
			...
			frameNo=tonumber(frameNo:match('%d+'))+1
